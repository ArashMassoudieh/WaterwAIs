...
void CTimeSeriesSet<T>::writetofile(char outputfile[]) {
	...
	if (!Fil) {
		cout << "File '" + std::string(outputfile) + "' cannot be opened!" << std::endl;
		return;
	}
...
template <class T>
CTimeSeriesSet<T>::CTimeSeriesSet(std::string _filename, bool varytime, uint64_t pos,
	...
	if (file_pos)
		file.seekg(file_pos, ios_base::beg);

	// Line checking function
	auto accept_line = [&var_size](auto& s) {
		if (s.empty()) // empty line
			return false;

		if (var_size > 0 && s.size() < var_size * 2)
			return false; // corrupted/partial line
	...
	// Assigning external names if any
	auto names_size = std::min(var_names.size(), names.size());
